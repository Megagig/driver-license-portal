...
export default function Signup({ paragraph, linkUrl, linkName }) {
  ...
  return (
    <form className="space-y-6 p-6" onSubmit={handleSubmit}>
      {error && (
        <p className="bg-red-100 text-red-700 py-2 px-4 rounded-md">{error}</p>
      )}
      <div className="">
        {fields.map((field) => (
          <Input
        ...
        <button
          ...
          type="submit"
          disabled={
            !isPasswordValid || !isPasswordMatch || !username || !isEmailValid
          }
        >
          {isSubmitting ? (
      ...
      <Modal isOpen={isOpen}>
        <SignUpResponse setIsModalOpen={setIsOpen} email={registeredEmail} />
      </Modal>
    </form>
  );
}
